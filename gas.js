/**
 * doPost関数をテストするためのデバッグ用関数
 * GASエディタ上で実行すると、doPostの処理をシミュレーションし、
 * 結果をログに出力
 */
function doPostTest() {
  // doPostに渡す模擬的なイベントオブジェクト「e」を作成します。
  // ここで検索したい語句を設定できます（例: "da"）
  const e = {
    postData: {
      contents: JSON.stringify({ message: "da" })
    }
  };
  
  Logger.log("--- doPostTest 開始 ---");
  const resultObject = doPost(e);
  const resultContents = resultObject.getContent();
  
  // 最終的なJSON出力をログに出力
  Logger.log("doPost 最終レスポンス: %s", resultContents);
  Logger.log("--- doPostTest 終了 ---");
}
...
function doPost(e) {
  try {
    ...
    // 検索語句を小文字化・トリム
    const searchTerm = data.message.toString().trim().toLowerCase();
    
    // 検索語句をログに出力
    Logger.log("検索語句: %s", searchTerm); 

    const ss = SpreadsheetApp.openById("1hDCPexxpLXBdg10yIw0jdXqpUzw4p4nNPOPkd-ACn10");
